# Thaler Testnet (preview): Running Nodes (Linux only)
...
- Start the the-query enclave app (in `tx-query-HW-debug/`), e.g.:

  ```
  RUST_LOG=info ./tx-query-app ipc://$HOME/enclave.socket 0.0.0.0:3443
  ```

- Start chain-abci, e.g.:

  ```
  RUST_LOG=info ./chain-abci --chain_id testnet-thaler-crypto-com-chain-42 --genesis_app_hash <TODO APP HASH> --enclave_server ipc://$HOME/enclave.socket --tx_query <EXTERNAL_IP/HOSTNAME>:3443
  ```
...
- `CRYPTO_CLIENT_TENDERMINT <YOUR FULL NODE, e.g. ws://localhost:26657/websocket, TODO: public deployed node>`
...
- Start chain-abci, e.g.:
  ...
  ```
  RUST_LOG=info ./chain-abci --chain_id testnet-thaler-crypto-com-chain-42 --genesis_app_hash <TODO APP HASH> --enclave_server ipc://$HOME/enclave.socket
  ```
