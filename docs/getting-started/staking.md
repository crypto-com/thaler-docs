...
If the NodeJoinTx transaction is valid AND this happens before next reward distribution AND the consensus pubkey (or associated Tendermint address) from NodeJoinTx transaction is different, the associated staked state's bonded amount is for both.
